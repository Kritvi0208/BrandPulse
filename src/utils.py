...
def _translate_text(article, lang_src='hi'):
    try:
        translated = GoogleTranslator(source=lang_src, target='en').translate(article)
        return translated if translated else article
    except Exception:
        return article
...
def translate(texts,hinglish=False):
    ...
    hinglish: boolean flag. set to True if you are translating hinglish
    Output:
    ...
    try:
      for batches in tqdm(text_batches):
          ...
          for batch in batches:
              ...
              time.sleep(interval) 
              translated_batch = _translate_text(batch, lang_src='hi')
